...
- 配置和清除确认使用拇指尖与食指尖距离除以手掌尺度，避免手离摄像头远近导致固定像素阈值失效。
- 绘画落笔和抬笔使用拇指尖与中指尖距离除以手掌尺度，食指尖只负责画笔坐标。
- 两类捏合都使用进入阈值和松开阈值两套阈值，形成滞回区间，减少临界状态抖动。
- 阈值集中定义为可调常量，实机测试后再确定最终值。
- 根据真实测试视频调整为：连续 2 帧满足进入阈值才确认捏合，连续 3 帧满足退出阈值才确认松开。
...
## 2026-06-25 绘画手势稳定性调整

- 配置选择和清除确认继续使用拇指 + 食指捏合，避免影响已有配置流程。
- 绘画落笔、保持和抬笔改为拇指 + 中指捏合，食指尖只负责画笔坐标。
- `GestureSnapshot` 同时输出配置捏合和绘画捏合状态，状态机仍只接收抽象的落笔/抬笔事件。
- 绘制引擎对单帧异常大跳变做限幅，再进入轻量低通平滑，减少关键点瞬时抖动造成的飞线。
